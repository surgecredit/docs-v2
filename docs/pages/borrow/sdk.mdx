...
The BTC/USD rate follows the Surge app: CoinGecko on mainnet (falling back to the Surge oracle), and the oracle only on signet - where the rate is set by Surge and does **not** track the market. `q.priceSource` tells you which answered. A 1.5% safety buffer is applied so a rate move between quoting and confirmation can't leave the draw under-collateralized; pass `bufferMultiplier: 1` for the raw minimum.
